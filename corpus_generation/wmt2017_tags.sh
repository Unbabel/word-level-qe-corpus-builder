...
out_edit_alignments="$out_temporal_folder/train.mt-pe.edit_alignments"
...
bash ./align.sh \
    ...
    $out_mt_alignments
echo "$out_temporal_folder/train.src-pe.alignments"
...
bash ./tercom.sh \
    ...
    $out_edit_alignments
echo "$out_temporal_folder/train.mt-pe.edits"

# WIP: Final check and gerate tags from here
exit
...
python ./generate_BAD_tags.py \
    ...
    $in_mt_file \
    $in_pe_file \
    $out_temporal_folder/train.src-pe.alignments \
    $out_temporal_folder/train.mt-pe.edit_alignments
